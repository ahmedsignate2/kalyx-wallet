...
/** Contexte de remplacement, tel que `broadcastSend` le rend. */
const ctx = (over: Partial<BitcoinPendingContext> = {}): BitcoinPendingContext => ({
  dest: OTHER,
  target: 50_000n,
  feeRate: 5,
  fee: 700n,
...
describe('usePendingBtc', () => {
  it('mémorise un envoi avec ses ENTRÉES — sans elles, aucun remplacement possible', () => {
     ...
     */
    usePendingBtc.getState().remember(ADDR, 'tx1', ctx());
  ...
  it('évince l\'originale quand un remplacement dépense les mêmes entrées', () => {
    ...
    usePendingBtc.getState().remember(ADDR, 'original', ctx({ feeRate: 5 }));
    usePendingBtc.getState().remember(ADDR, 'remplacement', ctx({ feeRate: 12 }));
  ...
  it('garde côte à côte deux envois qui ne partagent aucune entrée', () => {
    usePendingBtc.getState().remember(ADDR, 'un', ctx({ inputs: [utxo('a'.repeat(64))] }));
    usePendingBtc.getState().remember(ADDR, 'deux', ctx({ inputs: [utxo('b'.repeat(64))] }));
  ...
  it('bumpable ne rend que les transactions DE CE compte', () => {
    ...
    usePendingBtc.getState().remember(ADDR, 'mien', ctx({ inputs: [utxo('a'.repeat(64))] }));
    usePendingBtc.getState().remember(OTHER, 'autre', ctx({ inputs: [utxo('b'.repeat(64))] }));
  ...
  it('bumpable ignore une transaction sans entrées connues', () => {
    usePendingBtc.getState().remember(ADDR, 'tx1', ctx({ inputs: [] }));
