...
export interface Strings {
  ...
  scanUsage: string; scanRunning: string; scanNoData: string; scanNotToken: string; scanIsTxHash: string;
  addressReport: (flags: string[]) => string; addrFlags: Record<string, string>;
...
interface Words {
  ...
  scanUsage: string; scanRunning: string; scanNoData: string; scanNotToken: string; scanIsTxHash: string; addrReport: string; addrClean: string; addrFlagged: string; addrFlags: string[]; report: string; honeypot: string; sellable: string; buyTax: string; sellTax: string;
...
function build(w: Words): Strings {
  ...
  return {
    ...
    scanUsage: w.scanUsage, scanRunning: w.scanRunning, scanNoData: w.scanNoData, scanNotToken: w.scanNotToken, scanIsTxHash: w.scanIsTxHash,
    addressReport: (fl) => `<b>${w.addrReport}</b>\n\n${fl.length ? `🚨 ${w.addrFlagged}\n⚠️ ${fl.join('\n⚠️ ')}` : `✅ ${w.addrClean}`}\n\n<i>${w.disclaimer}</i>`,
    addrFlags: Object.fromEntries(ADDR_KEYS.map((k, i) => [k, w.addrFlags[i]])),
...
const ADDR_KEYS = ['phishing', 'blacklist', 'sanctioned', 'cybercrime', 'laundering', 'stealing', 'blackmail', 'darkweb', 'fakeKyc', 'honeypotRelated', 'mixer', 'mining', 'maliciousContracts'] as const;
...
const W: Record<Lang, Words> = {
  en: {
    ...
    scanUsage: 'Usage: /scan 0x… (EVM token or contract). Solana: /scan &lt;mint&gt;', scanRunning: '🔍 Checking with GoPlus Security…', scanNoData: 'No security data for this address (unknown or too new). Treat as risky.', scanNotToken: 'This is a wallet (or NFT) address, not a token. /scan checks a token’s contract or mint address.', scanIsTxHash: 'This is a transaction hash, not an address. /scan takes a token contract, a mint or a wallet address.', addrReport: 'Address reputation', addrClean: 'No report: this address is not flagged (phishing, blacklists, sanctions, theft).', addrFlagged: 'This address is flagged:', addrFlags: ['Phishing activity', 'On a blacklist', 'Sanctioned', 'Cybercrime', 'Money laundering', 'Theft / drainer', 'Blackmail', 'Dark-web transactions', 'Fake KYC', 'Linked to a honeypot', 'Mixer', 'Malicious mining', 'Has created malicious contracts'], report: 'Security report', honeypot: 'HONEYPOT — you may not be able to sell', sellable: 'Sellable', buyTax: 'Buy tax', sellTax: 'Sell tax', source: 'Source', verified: 'verified', notVerified: 'not verified', noFlags: 'No owner red flags found.', disclaimer: 'Automated check — not financial advice.',
  ...
  fr: {
    ...
    scanUsage: 'Usage : /scan 0x… (token ou contrat EVM). Solana : /scan &lt;mint&gt;', scanRunning: '🔍 Vérification avec GoPlus Security…', scanNoData: 'Aucune donnée de sécurité pour cette adresse (inconnue ou trop récente). À considérer comme risquée.', scanNotToken: 'C’est une adresse de wallet (ou de NFT), pas un token. /scan vérifie l’adresse de contrat ou le mint d’un token.', scanIsTxHash: 'C\'est un hash de transaction, pas une adresse. /scan prend un contrat de token, un mint ou une adresse de wallet.', addrReport: 'Réputation de l\'adresse', addrClean: 'Aucun signalement : cette adresse n\'est pas marquée (phishing, listes noires, sanctions, vol).', addrFlagged: 'Cette adresse est signalée :', addrFlags: ['Activité de phishing', 'Sur une liste noire', 'Sanctionnée', 'Cybercriminalité', 'Blanchiment', 'Vol / drainer', 'Chantage', 'Transactions dark web', 'Faux KYC', 'Liée à un honeypot', 'Mixeur', 'Minage malveillant', 'A créé des contrats malveillants'], report: 'Rapport de sécurité', honeypot: 'HONEYPOT — tu pourrais ne pas pouvoir revendre', sellable: 'Revendable', buyTax: 'Taxe achat', sellTax: 'Taxe vente', source: 'Code', verified: 'vérifié', notVerified: 'non vérifié', noFlags: 'Aucun droit dangereux du owner détecté.', disclaimer: 'Vérification automatique — pas un conseil financier.',
  ...
  es: {
    ...
    scanUsage: 'Uso: /scan 0x… (token o contrato EVM). Solana: /scan &lt;mint&gt;', scanRunning: '🔍 Comprobando con GoPlus Security…', scanNoData: 'Sin datos de seguridad para esta dirección (desconocida o muy nueva). Trátala como riesgosa.', scanNotToken: 'Es una dirección de cartera (o NFT), no un token. /scan verifica el contrato o mint de un token.', scanIsTxHash: 'Es un hash de transacción, no una dirección. /scan acepta un contrato de token, un mint o una dirección de cartera.', addrReport: 'Reputación de la dirección', addrClean: 'Sin reportes: esta dirección no está marcada (phishing, listas negras, sanciones, robo).', addrFlagged: 'Esta dirección está marcada:', addrFlags: ['Actividad de phishing', 'En lista negra', 'Sancionada', 'Cibercrimen', 'Blanqueo de dinero', 'Robo / drainer', 'Chantaje', 'Transacciones en la dark web', 'KYC falso', 'Vinculada a un honeypot', 'Mezclador', 'Minería maliciosa', 'Ha creado contratos maliciosos'], report: 'Informe de seguridad', honeypot: 'HONEYPOT — quizá no puedas vender', sellable: 'Vendible', buyTax: 'Tasa compra', sellTax: 'Tasa venta', source: 'Código', verified: 'verificado', notVerified: 'no verificado', noFlags: 'Sin permisos peligrosos del owner.', disclaimer: 'Comprobación automática — no es asesoría financiera.',
  ...
  pt: {
    ...
    scanUsage: 'Uso: /scan 0x… (token ou contrato EVM). Solana: /scan &lt;mint&gt;', scanRunning: '🔍 A verificar com GoPlus Security…', scanNoData: 'Sem dados de segurança para este endereço (desconhecido ou muito recente). Considera-o arriscado.', scanNotToken: 'É um endereço de carteira (ou NFT), não um token. /scan verifica o contrato ou mint de um token.', scanIsTxHash: 'É um hash de transação, não um endereço. /scan aceita um contrato de token, um mint ou um endereço de carteira.', addrReport: 'Reputação do endereço', addrClean: 'Sem sinalizações: este endereço não está marcado (phishing, listas negras, sanções, roubo).', addrFlagged: 'Este endereço está sinalizado:', addrFlags: ['Atividade de phishing', 'Em lista negra', 'Sancionado', 'Cibercrime', 'Branqueamento', 'Roubo / drainer', 'Chantagem', 'Transações na dark web', 'KYC falso', 'Ligado a um honeypot', 'Mixer', 'Mineração maliciosa', 'Criou contratos maliciosos'], report: 'Relatório de segurança', honeypot: 'HONEYPOT — podes não conseguir vender', sellable: 'Vendável', buyTax: 'Taxa compra', sellTax: 'Taxa venda', source: 'Código', verified: 'verificado', notVerified: 'não verificado', noFlags: 'Sem permissões perigosas do owner.', disclaimer: 'Verificação automática — não é aconselhamento financeiro.',
  ...
  de: {
    ...
    scanUsage: 'Verwendung: /scan 0x… (EVM-Token oder -Vertrag). Solana: /scan &lt;mint&gt;', scanRunning: '🔍 Prüfung mit GoPlus Security…', scanNoData: 'Keine Sicherheitsdaten für diese Adresse (unbekannt oder zu neu). Als riskant behandeln.', scanNotToken: 'Das ist eine Wallet- (oder NFT-)Adresse, kein Token. /scan prüft die Vertrags- oder Mint-Adresse eines Tokens.', scanIsTxHash: 'Das ist ein Transaktions-Hash, keine Adresse. /scan nimmt einen Token-Vertrag, einen Mint oder eine Wallet-Adresse.', addrReport: 'Adress-Reputation', addrClean: 'Keine Meldung: Diese Adresse ist nicht markiert (Phishing, Blacklists, Sanktionen, Diebstahl).', addrFlagged: 'Diese Adresse ist markiert:', addrFlags: ['Phishing-Aktivität', 'Auf einer Blacklist', 'Sanktioniert', 'Cyberkriminalität', 'Geldwäsche', 'Diebstahl / Drainer', 'Erpressung', 'Darknet-Transaktionen', 'Gefälschtes KYC', 'Mit Honeypot verbunden', 'Mixer', 'Bösartiges Mining', 'Hat bösartige Verträge erstellt'], report: 'Sicherheitsbericht', honeypot: 'HONEYPOT — du kannst möglicherweise nicht verkaufen', sellable: 'Verkaufbar', buyTax: 'Kaufsteuer', sellTax: 'Verkaufssteuer', source: 'Code', verified: 'verifiziert', notVerified: 'nicht verifiziert', noFlags: 'Keine gefährlichen Owner-Rechte gefunden.', disclaimer: 'Automatische Prüfung — keine Finanzberatung.',
  ...
  nl: {
    ...
    scanUsage: 'Gebruik: /scan 0x… (EVM-token of -contract). Solana: /scan &lt;mint&gt;', scanRunning: '🔍 Controle met GoPlus Security…', scanNoData: 'Geen beveiligingsgegevens voor dit adres (onbekend of te nieuw). Behandel als risicovol.', scanNotToken: 'Dit is een wallet- (of NFT-)adres, geen token. /scan controleert het contract- of mint-adres van een token.', scanIsTxHash: 'Dit is een transactie-hash, geen adres. /scan neemt een tokencontract, een mint of een walletadres.', addrReport: 'Reputatie van het adres', addrClean: 'Geen melding: dit adres is niet gemarkeerd (phishing, blacklists, sancties, diefstal).', addrFlagged: 'Dit adres is gemarkeerd:', addrFlags: ['Phishingactiviteit', 'Op een blacklist', 'Gesanctioneerd', 'Cybercriminaliteit', 'Witwassen', 'Diefstal / drainer', 'Chantage', 'Darkweb-transacties', 'Vals KYC', 'Gekoppeld aan honeypot', 'Mixer', 'Kwaadaardige mining', 'Heeft kwaadaardige contracten gemaakt'], report: 'Beveiligingsrapport', honeypot: 'HONEYPOT — verkopen lukt misschien niet', sellable: 'Verkoopbaar', buyTax: 'Koopbelasting', sellTax: 'Verkoopbelasting', source: 'Code', verified: 'geverifieerd', notVerified: 'niet geverifieerd', noFlags: 'Geen gevaarlijke ownerrechten gevonden.', disclaimer: 'Automatische controle — geen financieel advies.',
  ...
  pl: {
    ...
    scanUsage: 'Użycie: /scan 0x… (token lub kontrakt EVM). Solana: /scan &lt;mint&gt;', scanRunning: '🔍 Sprawdzanie w GoPlus Security…', scanNoData: 'Brak danych bezpieczeństwa dla tego adresu (nieznany lub zbyt nowy). Traktuj jako ryzykowny.', scanNotToken: 'To adres portfela (lub NFT), nie token. /scan sprawdza adres kontraktu lub mint tokena.', scanIsTxHash: 'To hash transakcji, nie adres. /scan przyjmuje kontrakt tokena, mint lub adres portfela.', addrReport: 'Reputacja adresu', addrClean: 'Brak zgłoszeń: ten adres nie jest oznaczony (phishing, czarne listy, sankcje, kradzież).', addrFlagged: 'Ten adres jest oznaczony:', addrFlags: ['Aktywność phishingowa', 'Na czarnej liście', 'Objęty sankcjami', 'Cyberprzestępczość', 'Pranie pieniędzy', 'Kradzież / drainer', 'Szantaż', 'Transakcje w darknecie', 'Fałszywe KYC', 'Powiązany z honeypotem', 'Mikser', 'Złośliwe kopanie', 'Tworzył złośliwe kontrakty'], report: 'Raport bezpieczeństwa', honeypot: 'HONEYPOT — możesz nie być w stanie sprzedać', sellable: 'Sprzedawalny', buyTax: 'Podatek kupna', sellTax: 'Podatek sprzedaży', source: 'Kod', verified: 'zweryfikowany', notVerified: 'niezweryfikowany', noFlags: 'Brak niebezpiecznych uprawnień właściciela.', disclaimer: 'Kontrola automatyczna — to nie porada finansowa.',
  ...
  tr: {
    ...
    scanUsage: 'Kullanım: /scan 0x… (EVM token veya kontrat). Solana: /scan &lt;mint&gt;', scanRunning: '🔍 GoPlus Security ile kontrol ediliyor…', scanNoData: 'Bu adres için güvenlik verisi yok (bilinmiyor veya çok yeni). Riskli say.', scanNotToken: 'Bu bir cüzdan (veya NFT) adresi, token değil. /scan bir tokenın kontrat veya mint adresini kontrol eder.', scanIsTxHash: 'Bu bir işlem hash\'i, adres değil. /scan bir token kontratı, mint veya cüzdan adresi alır.', addrReport: 'Adres itibarı', addrClean: 'Bildirim yok: bu adres işaretlenmemiş (oltalama, kara listeler, yaptırımlar, hırsızlık).', addrFlagged: 'Bu adres işaretli:', addrFlags: ['Oltalama etkinliği', 'Kara listede', 'Yaptırımlı', 'Siber suç', 'Kara para aklama', 'Hırsızlık / drainer', 'Şantaj', 'Dark web işlemleri', 'Sahte KYC', 'Honeypot ile bağlantılı', 'Karıştırıcı', 'Kötü amaçlı madencilik', 'Kötü amaçlı kontratlar oluşturmuş'], report: 'Güvenlik raporu', honeypot: 'HONEYPOT — satamayabilirsin', sellable: 'Satılabilir', buyTax: 'Alış vergisi', sellTax: 'Satış vergisi', source: 'Kod', verified: 'doğrulanmış', notVerified: 'doğrulanmamış', noFlags: 'Tehlikeli owner yetkisi bulunmadı.', disclaimer: 'Otomatik kontrol — finansal tavsiye değildir.',
  ...
  ru: {
    ...
    scanUsage: 'Использование: /scan 0x… (токен или контракт EVM). Solana: /scan &lt;mint&gt;', scanRunning: '🔍 Проверка через GoPlus Security…', scanNoData: 'Нет данных безопасности для этого адреса (неизвестен или слишком новый). Считайте рискованным.', scanNotToken: 'Это адрес кошелька (или NFT), а не токен. /scan проверяет адрес контракта или mint токена.', scanIsTxHash: 'Это хеш транзакции, а не адрес. /scan принимает контракт токена, mint или адрес кошелька.', addrReport: 'Репутация адреса', addrClean: 'Без пометок: этот адрес не отмечен (фишинг, чёрные списки, санкции, кража).', addrFlagged: 'Этот адрес отмечен:', addrFlags: ['Фишинговая активность', 'В чёрном списке', 'Под санкциями', 'Киберпреступность', 'Отмывание денег', 'Кража / дрейнер', 'Шантаж', 'Транзакции в даркнете', 'Поддельный KYC', 'Связан с honeypot', 'Миксер', 'Вредоносный майнинг', 'Создавал вредоносные контракты'], report: 'Отчёт о безопасности', honeypot: 'HONEYPOT — продать может не получиться', sellable: 'Можно продать', buyTax: 'Налог на покупку', sellTax: 'Налог на продажу', source: 'Код', verified: 'проверен', notVerified: 'не проверен', noFlags: 'Опасных прав владельца не найдено.', disclaimer: 'Автоматическая проверка — не финансовый совет.',
  ...
  ar: {
    ...
    scanUsage: 'الاستخدام: /scan 0x… (رمز أو عقد EVM). Solana: /scan &lt;mint&gt;', scanRunning: '🔍 يجري الفحص عبر GoPlus Security…', scanNoData: 'لا بيانات أمان لهذا العنوان (غير معروف أو جديد جدًا). اعتبره خطرًا.', scanNotToken: 'هذا عنوان محفظة (أو NFT) وليس رمزًا. /scan يفحص عنوان عقد أو mint الرمز.', scanIsTxHash: 'هذا تجزئة معاملة وليس عنوانًا. /scan يقبل عقد رمز أو mint أو عنوان محفظة.', addrReport: 'سمعة العنوان', addrClean: 'لا بلاغات: هذا العنوان غير مُعلَّم (تصيّد، قوائم سوداء، عقوبات، سرقة).', addrFlagged: 'هذا العنوان مُعلَّم:', addrFlags: ['نشاط تصيّد', 'في قائمة سوداء', 'خاضع لعقوبات', 'جرائم إلكترونية', 'غسل أموال', 'سرقة / drainer', 'ابتزاز', 'معاملات دارك ويب', 'KYC مزيّف', 'مرتبط بـ honeypot', 'خلّاط', 'تعدين خبيث', 'أنشأ عقودًا خبيثة'], report: 'تقرير الأمان', honeypot: 'HONEYPOT — قد لا تستطيع البيع', sellable: 'قابل للبيع', buyTax: 'ضريبة الشراء', sellTax: 'ضريبة البيع', source: 'الكود', verified: 'مُتحقَّق', notVerified: 'غير مُتحقَّق', noFlags: 'لا صلاحيات خطرة للمالك.', disclaimer: 'فحص آلي — ليس نصيحة مالية.',
  ...
  hi: {
    ...
    scanUsage: 'उपयोग: /scan 0x… (EVM टोकन या कॉन्ट्रैक्ट)। Solana: /scan &lt;mint&gt;', scanRunning: '🔍 GoPlus Security से जाँच हो रही है…', scanNoData: 'इस पते के लिए सुरक्षा डेटा नहीं (अज्ञात या बहुत नया)। जोखिमपूर्ण मानें।', scanNotToken: 'यह वॉलेट (या NFT) पता है, टोकन नहीं। /scan टोकन के कॉन्ट्रैक्ट या mint पते की जाँच करता है।', scanIsTxHash: 'यह लेन-देन हैश है, पता नहीं। /scan टोकन कॉन्ट्रैक्ट, mint या वॉलेट पता लेता है।', addrReport: 'पते की प्रतिष्ठा', addrClean: 'कोई रिपोर्ट नहीं: यह पता चिह्नित नहीं है (फ़िशिंग, ब्लैकलिस्ट, प्रतिबंध, चोरी)।', addrFlagged: 'यह पता चिह्नित है:', addrFlags: ['फ़िशिंग गतिविधि', 'ब्लैकलिस्ट में', 'प्रतिबंधित', 'साइबर अपराध', 'धन शोधन', 'चोरी / drainer', 'ब्लैकमेल', 'डार्क वेब लेन-देन', 'नकली KYC', 'honeypot से जुड़ा', 'मिक्सर', 'दुर्भावनापूर्ण माइनिंग', 'दुर्भावनापूर्ण कॉन्ट्रैक्ट बनाए'], report: 'सुरक्षा रिपोर्ट', honeypot: 'HONEYPOT — आप बेच नहीं पाएँगे शायद', sellable: 'बेचा जा सकता है', buyTax: 'ख़रीद कर', sellTax: 'बिक्री कर', source: 'कोड', verified: 'सत्यापित', notVerified: 'असत्यापित', noFlags: 'मालिक के ख़तरनाक अधिकार नहीं मिले।', disclaimer: 'स्वचालित जाँच — वित्तीय सलाह नहीं।',
  ...
  zh: {
    ...
    scanUsage: '用法：/scan 0x…（EVM 代币或合约）。Solana：/scan &lt;mint&gt;', scanRunning: '🔍 正在通过 GoPlus Security 检查…', scanNoData: '该地址没有安全数据（未知或太新），请视为高风险。', scanNotToken: '这是钱包（或 NFT）地址，不是代币。/scan 检查代币的合约或 mint 地址。', scanIsTxHash: '这是交易哈希，不是地址。/scan 接受代币合约、mint 或钱包地址。', addrReport: '地址信誉', addrClean: '无举报：该地址未被标记（钓鱼、黑名单、制裁、盗窃）。', addrFlagged: '该地址已被标记：', addrFlags: ['钓鱼活动', '列入黑名单', '受制裁', '网络犯罪', '洗钱', '盗窃 / drainer', '勒索', '暗网交易', '虚假 KYC', '与蜜罐相关', '混币器', '恶意挖矿', '创建过恶意合约'], report: '安全报告', honeypot: '蜜罐 — 你可能无法卖出', sellable: '可卖出', buyTax: '买入税', sellTax: '卖出税', source: '代码', verified: '已验证', notVerified: '未验证', noFlags: '未发现危险的所有者权限。', disclaimer: '自动检测 — 不构成投资建议。',
  ...
  ja: {
    ...
    scanUsage: '使い方：/scan 0x…（EVM トークンまたはコントラクト）。Solana：/scan &lt;mint&gt;', scanRunning: '🔍 GoPlus Security で確認中…', scanNoData: 'このアドレスの安全性データがありません（不明または新しすぎます）。リスクありとして扱ってください。', scanNotToken: 'これはウォレット（または NFT）のアドレスで、トークンではありません。/scan はトークンのコントラクト／ミントアドレスを確認します。', scanIsTxHash: 'これはトランザクションハッシュで、アドレスではありません。/scan はトークンコントラクト、ミント、ウォレットアドレスを受け付けます。', addrReport: 'アドレスの評判', addrClean: '報告なし：このアドレスはフラグされていません（フィッシング、ブラックリスト、制裁、盗難）。', addrFlagged: 'このアドレスはフラグ付きです：', addrFlags: ['フィッシング活動', 'ブラックリスト掲載', '制裁対象', 'サイバー犯罪', 'マネーロンダリング', '窃盗 / ドレイナー', '脅迫', 'ダークウェブ取引', '偽の KYC', 'ハニーポット関連', 'ミキサー', '悪意あるマイニング', '悪意あるコントラクトを作成'], report: 'セキュリティレポート', honeypot: 'ハニーポット — 売却できない可能性', sellable: '売却可能', buyTax: '購入税', sellTax: '売却税', source: 'コード', verified: '検証済み', notVerified: '未検証', noFlags: '危険なオーナー権限は見つかりません。', disclaimer: '自動チェック — 投資助言ではありません。',
  ...
  ko: {
    ...
    scanUsage: '사용법: /scan 0x… (EVM 토큰 또는 컨트랙트). Solana: /scan &lt;mint&gt;', scanRunning: '🔍 GoPlus Security로 확인 중…', scanNoData: '이 주소의 보안 데이터가 없습니다 (알 수 없거나 너무 새로움). 위험한 것으로 간주하세요.', scanNotToken: '이것은 지갑(또는 NFT) 주소이며 토큰이 아닙니다. /scan은 토큰의 컨트랙트 또는 mint 주소를 확인합니다.', scanIsTxHash: '이것은 거래 해시이며 주소가 아닙니다. /scan은 토큰 컨트랙트, mint 또는 지갑 주소를 받습니다.', addrReport: '주소 평판', addrClean: '신고 없음: 이 주소는 표시되지 않았습니다 (피싱, 블랙리스트, 제재, 도난).', addrFlagged: '이 주소는 표시됨:', addrFlags: ['피싱 활동', '블랙리스트 등재', '제재 대상', '사이버 범죄', '자금 세탁', '도난 / 드레이너', '협박', '다크웹 거래', '가짜 KYC', '허니팟 연관', '믹서', '악성 채굴', '악성 컨트랙트 생성'], report: '보안 보고서', honeypot: '허니팟 — 매도가 불가능할 수 있음', sellable: '매도 가능', buyTax: '매수 세율', sellTax: '매도 세율', source: '코드', verified: '검증됨', notVerified: '미검증', noFlags: '위험한 소유자 권한이 없습니다.', disclaimer: '자동 검사 — 투자 조언이 아닙니다.',
