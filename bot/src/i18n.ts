...
export interface Strings {
  ...
  scanUsage: string; scanRunning: string; scanNoData: string;
  ...
  watchUsage: string; watchInvalid: string; watchLimit: string; watchAdded: (a: string, label: string) => string; watchExists: string; watchNone: string;
  watchList: (rows: { address: string; label: string | null }[]) => string; unwatchUsage: string; unwatched: string;
  ...
  txIn: (label: string, amount: string, from: string, link: string) => string; txOut: (label: string, amount: string, to: string, link: string) => string;
...
interface Words {
  ...
  cmdApp: string; cmdPrice: string; cmdGas: string; cmdScan: string; cmdWatch: string; cmdAlert: string; cmdHelp: string; cmdLang: string;
  openApp: string; installApp: string; site: string; download: string; support: string; changeLanguage: string;
  ...
  scanUsage: string; scanRunning: string; scanNoData: string; report: string; honeypot: string; sellable: string; buyTax: string; sellTax: string;
  source: string; verified: string; notVerified: string; noFlags: string; disclaimer: string;
  flags: Record<string, string>;
  watchUsage: string; watchInvalid: string; watchLimit: string; watchAdded: string; watchExists: string; watchNone: string; watchListTitle: string; unwatchUsage: string; unwatched: string;
  alertUsage: string; alertAdded: string; above: string; below: string; alertsNone: string; alertsTitle: string; alertFired: string;
  received: string; sent: string; from: string; to: string; on: string;
  rateLimited: string; error: string; unknown: string;
}
...
function build(w: Words): Strings {
  ...
  return {
    ...
    scanUsage: w.scanUsage, scanRunning: w.scanRunning, scanNoData: w.scanNoData,
    ...
    scanFlags: w.flags,
    watchUsage: w.watchUsage, watchInvalid: w.watchInvalid, watchLimit: w.watchLimit,
    watchAdded: (a, label) => `👀 ${fill(w.watchAdded, { a: `<code>${a}</code>${label ? ` (${label})` : ''}` })}`,
    watchExists: w.watchExists, watchNone: w.watchNone,
    watchList: (rows) => `<b>${w.watchListTitle}</b>\n` + rows.map((r) => `• <code>${r.address}</code>${r.label ? ` — ${r.label}` : ''}`).join('\n'),
    unwatchUsage: w.unwatchUsage, unwatched: w.unwatched,
    ...
    txIn: (label, amount, from, link) => `🔔 <b>${w.received}</b> ${w.on} ${label}\n+${amount}\n${w.from} <code>${from}</code>\n${link}`,
    txOut: (label, amount, to, link) => `🔔 <b>${w.sent}</b> ${w.on} ${label}\n−${amount}\n${w.to} <code>${to}</code>\n${link}`,
...
const W: Record<Lang, Words> = {
  en: {
    ...
    cmdApp: 'open the wallet inside Telegram', cmdPrice: 'live price', cmdGas: 'network fees right now', cmdScan: 'honeypot / scam check', cmdWatch: 'a message on every transaction', cmdAlert: 'price alert', cmdHelp: 'how it works', cmdLang: 'change language',
    ...
    scanUsage: 'Usage: /scan 0x… (EVM token or contract). Solana: /scan &lt;mint&gt;', scanRunning: '🔍 Checking with GoPlus Security…', scanNoData: 'No security data for this address (unknown or too new). Treat as risky.', report: 'Security report', honeypot: 'HONEYPOT — you may not be able to sell', sellable: 'Sellable', buyTax: 'Buy tax', sellTax: 'Sell tax', source: 'Source', verified: 'verified', notVerified: 'not verified', noFlags: 'No owner red flags found.', disclaimer: 'Automated check — not financial advice.',
    ...
    watchUsage: 'Usage: /watch 0x… [label] — EVM, Solana or Bitcoin address.', watchInvalid: 'That does not look like a valid address.', watchLimit: 'You already follow 10 addresses. Remove one with /unwatch.', watchAdded: 'Following {a}. You will get a message on each transaction.', watchExists: 'You already follow this address.', watchNone: 'You follow no address yet. /watch 0x…', watchListTitle: 'Followed addresses', unwatchUsage: 'Usage: /unwatch 0x…', unwatched: 'Removed.',
    ...
    received: 'Received', sent: 'Sent', from: 'From', to: 'To', on: 'on',
  ...
  fr: {
    ...
    cmdApp: 'ouvrir le wallet dans Telegram', cmdPrice: 'cours en direct', cmdGas: 'frais réseau maintenant', cmdScan: 'détection honeypot / arnaque', cmdWatch: 'un message à chaque transaction', cmdAlert: 'alerte de prix', cmdHelp: 'comment ça marche', cmdLang: 'changer de langue',
    ...
    watchUsage: 'Usage : /watch 0x… [libellé] — adresse EVM, Solana ou Bitcoin.', watchInvalid: 'Ça ne ressemble pas à une adresse valide.', watchLimit: 'Tu suis déjà 10 adresses. Retire-en une avec /unwatch.', watchAdded: 'Suivi de {a}. Tu recevras un message à chaque transaction.', watchExists: 'Tu suis déjà cette adresse.', watchNone: 'Tu ne suis encore aucune adresse. /watch 0x…', watchListTitle: 'Adresses suivies', unwatchUsage: 'Usage : /unwatch 0x…', unwatched: 'Retirée.',
    ...
    received: 'Reçu', sent: 'Envoyé', from: 'De', to: 'Vers', on: 'sur',
  ...
  es: {
    ...
    cmdApp: 'abrir la cartera dentro de Telegram', cmdPrice: 'precio en vivo', cmdGas: 'comisiones de red ahora', cmdScan: 'detección de honeypot / estafa', cmdWatch: 'un mensaje por cada transacción', cmdAlert: 'alerta de precio', cmdHelp: 'cómo funciona', cmdLang: 'cambiar idioma',
    ...
    watchUsage: 'Uso: /watch 0x… [etiqueta] — dirección EVM, Solana o Bitcoin.', watchInvalid: 'Eso no parece una dirección válida.', watchLimit: 'Ya sigues 10 direcciones. Quita una con /unwatch.', watchAdded: 'Siguiendo {a}. Recibirás un mensaje por cada transacción.', watchExists: 'Ya sigues esta dirección.', watchNone: 'Aún no sigues ninguna dirección. /watch 0x…', watchListTitle: 'Direcciones seguidas', unwatchUsage: 'Uso: /unwatch 0x…', unwatched: 'Eliminada.',
    ...
    received: 'Recibido', sent: 'Enviado', from: 'De', to: 'A', on: 'en',
  ...
  pt: {
    ...
    cmdApp: 'abrir a carteira dentro do Telegram', cmdPrice: 'preço em direto', cmdGas: 'taxas de rede agora', cmdScan: 'deteção de honeypot / fraude', cmdWatch: 'uma mensagem a cada transação', cmdAlert: 'alerta de preço', cmdHelp: 'como funciona', cmdLang: 'mudar idioma',
    ...
    watchUsage: 'Uso: /watch 0x… [etiqueta] — endereço EVM, Solana ou Bitcoin.', watchInvalid: 'Isso não parece um endereço válido.', watchLimit: 'Já segues 10 endereços. Remove um com /unwatch.', watchAdded: 'A seguir {a}. Receberás uma mensagem a cada transação.', watchExists: 'Já segues este endereço.', watchNone: 'Ainda não segues nenhum endereço. /watch 0x…', watchListTitle: 'Endereços seguidos', unwatchUsage: 'Uso: /unwatch 0x…', unwatched: 'Removido.',
    ...
    received: 'Recebido', sent: 'Enviado', from: 'De', to: 'Para', on: 'em',
  ...
  de: {
    ...
    cmdApp: 'Wallet in Telegram öffnen', cmdPrice: 'Live-Kurs', cmdGas: 'aktuelle Netzwerkgebühren', cmdScan: 'Honeypot-/Betrugsprüfung', cmdWatch: 'eine Nachricht bei jeder Transaktion', cmdAlert: 'Preisalarm', cmdHelp: 'so funktioniert es', cmdLang: 'Sprache ändern',
    ...
    watchUsage: 'Verwendung: /watch 0x… [Label] — EVM-, Solana- oder Bitcoin-Adresse.', watchInvalid: 'Das sieht nicht wie eine gültige Adresse aus.', watchLimit: 'Du folgst bereits 10 Adressen. Entferne eine mit /unwatch.', watchAdded: 'Folge {a}. Du bekommst bei jeder Transaktion eine Nachricht.', watchExists: 'Du folgst dieser Adresse bereits.', watchNone: 'Du folgst noch keiner Adresse. /watch 0x…', watchListTitle: 'Verfolgte Adressen', unwatchUsage: 'Verwendung: /unwatch 0x…', unwatched: 'Entfernt.',
    ...
    received: 'Empfangen', sent: 'Gesendet', from: 'Von', to: 'An', on: 'auf',
  ...
  it: {
    ...
    cmdApp: 'apri il wallet dentro Telegram', cmdPrice: 'prezzo in tempo reale', cmdGas: 'commissioni di rete adesso', cmdScan: 'controllo honeypot / truffa', cmdWatch: 'un messaggio a ogni transazione', cmdAlert: 'avviso di prezzo', cmdHelp: 'come funziona', cmdLang: 'cambia lingua',
    ...
    watchUsage: 'Uso: /watch 0x… [etichetta] — indirizzo EVM, Solana o Bitcoin.', watchInvalid: 'Non sembra un indirizzo valido.', watchLimit: 'Segui già 10 indirizzi. Rimuovine uno con /unwatch.', watchAdded: 'Seguo {a}. Riceverai un messaggio a ogni transazione.', watchExists: 'Segui già questo indirizzo.', watchNone: 'Non segui ancora nessun indirizzo. /watch 0x…', watchListTitle: 'Indirizzi seguiti', unwatchUsage: 'Uso: /unwatch 0x…', unwatched: 'Rimosso.',
    ...
    received: 'Ricevuto', sent: 'Inviato', from: 'Da', to: 'A', on: 'su',
  ...
  nl: {
    ...
    cmdApp: 'open de wallet in Telegram', cmdPrice: 'live koers', cmdGas: 'netwerkkosten nu', cmdScan: 'honeypot-/scamcontrole', cmdWatch: 'een bericht bij elke transactie', cmdAlert: 'prijsalarm', cmdHelp: 'hoe het werkt', cmdLang: 'taal wijzigen',
    ...
    watchUsage: 'Gebruik: /watch 0x… [label] — EVM-, Solana- of Bitcoin-adres.', watchInvalid: 'Dat lijkt geen geldig adres.', watchLimit: 'Je volgt al 10 adressen. Verwijder er een met /unwatch.', watchAdded: 'Volgt {a}. Je krijgt bij elke transactie een bericht.', watchExists: 'Je volgt dit adres al.', watchNone: 'Je volgt nog geen adres. /watch 0x…', watchListTitle: 'Gevolgde adressen', unwatchUsage: 'Gebruik: /unwatch 0x…', unwatched: 'Verwijderd.',
    ...
    received: 'Ontvangen', sent: 'Verzonden', from: 'Van', to: 'Naar', on: 'op',
  ...
  pl: {
    ...
    cmdApp: 'otwórz portfel w Telegramie', cmdPrice: 'kurs na żywo', cmdGas: 'opłaty sieciowe teraz', cmdScan: 'wykrywanie honeypot / oszustw', cmdWatch: 'wiadomość przy każdej transakcji', cmdAlert: 'alert cenowy', cmdHelp: 'jak to działa', cmdLang: 'zmień język',
    ...
    scanUsage: 'Użycie: /scan 0x… (token lub kontrakt EVM). Solana: /scan &lt;mint&gt;', scanRunning: '🔍 Sprawdzanie w GoPlus Security…', scanNoData: 'Brak danych bezpieczeństwa dla tego adresu (nieznany lub zbyt nowy). Traktuj jako ryzykowny.', report: 'Raport bezpieczeństwa', honeypot: 'HONEYPOT — możesz nie być w stanie sprzedać', sellable: 'Sprzedawalny', buyTax: 'Podatek kupna', sellTax: 'Podatek sprzedaży', source: 'Kod', verified: 'zweryfikowany', notVerified: 'niezweryfikowany', noFlags: 'Brak niebezpiecznych uprawnień właściciela.', disclaimer: 'Kontrola automatyczna — to nie porada finansowa.',
    ...
    watchUsage: 'Użycie: /watch 0x… [etykieta] — adres EVM, Solana lub Bitcoin.', watchInvalid: 'To nie wygląda na poprawny adres.', watchLimit: 'Śledzisz już 10 adresów. Usuń jeden przez /unwatch.', watchAdded: 'Śledzę {a}. Dostaniesz wiadomość przy każdej transakcji.', watchExists: 'Już śledzisz ten adres.', watchNone: 'Nie śledzisz jeszcze żadnego adresu. /watch 0x…', watchListTitle: 'Śledzone adresy', unwatchUsage: 'Użycie: /unwatch 0x…', unwatched: 'Usunięto.',
    ...
    received: 'Otrzymano', sent: 'Wysłano', from: 'Od', to: 'Do', on: 'na',
  ...
  tr: {
    ...
    cmdApp: 'cüzdanı Telegram içinde aç', cmdPrice: 'canlı fiyat', cmdGas: 'anlık ağ ücretleri', cmdScan: 'honeypot / dolandırıcılık kontrolü', cmdWatch: 'her işlemde bir mesaj', cmdAlert: 'fiyat alarmı', cmdHelp: 'nasıl çalışır', cmdLang: 'dili değiştir',
    ...
    watchUsage: 'Kullanım: /watch 0x… [etiket] — EVM, Solana veya Bitcoin adresi.', watchInvalid: 'Bu geçerli bir adrese benzemiyor.', watchLimit: 'Zaten 10 adres takip ediyorsun. /unwatch ile birini kaldır.', watchAdded: '{a} takip ediliyor. Her işlemde mesaj alacaksın.', watchExists: 'Bu adresi zaten takip ediyorsun.', watchNone: 'Henüz adres takip etmiyorsun. /watch 0x…', watchListTitle: 'Takip edilen adresler', unwatchUsage: 'Kullanım: /unwatch 0x…', unwatched: 'Kaldırıldı.',
    ...
    received: 'Alındı', sent: 'Gönderildi', from: 'Kimden', to: 'Kime', on: '·',
  ...
  ru: {
    ...
    cmdApp: 'открыть кошелёк в Telegram', cmdPrice: 'курс в реальном времени', cmdGas: 'комиссии сети сейчас', cmdScan: 'проверка на honeypot / скам', cmdWatch: 'сообщение о каждой транзакции', cmdAlert: 'ценовое оповещение', cmdHelp: 'как это работает', cmdLang: 'сменить язык',
    ...
    watchUsage: 'Использование: /watch 0x… [метка] — адрес EVM, Solana или Bitcoin.', watchInvalid: 'Это не похоже на корректный адрес.', watchLimit: 'Вы уже следите за 10 адресами. Удалите один через /unwatch.', watchAdded: 'Слежу за {a}. Вы получите сообщение о каждой транзакции.', watchExists: 'Вы уже следите за этим адресом.', watchNone: 'Вы пока не следите ни за одним адресом. /watch 0x…', watchListTitle: 'Отслеживаемые адреса', unwatchUsage: 'Использование: /unwatch 0x…', unwatched: 'Удалено.',
    ...
    received: 'Получено', sent: 'Отправлено', from: 'От', to: 'Кому', on: 'на',
  ...
  ar: {
    ...
    cmdApp: 'افتح المحفظة داخل Telegram', cmdPrice: 'السعر المباشر', cmdGas: 'رسوم الشبكة الآن', cmdScan: 'كشف الـ honeypot / الاحتيال', cmdWatch: 'رسالة عند كل معاملة', cmdAlert: 'تنبيه سعر', cmdHelp: 'كيف يعمل', cmdLang: 'تغيير اللغة',
    ...
    scanUsage: 'الاستخدام: /scan 0x… (رمز أو عقد EVM). Solana: /scan &lt;mint&gt;', scanRunning: '🔍 يجري الفحص عبر GoPlus Security…', scanNoData: 'لا بيانات أمان لهذا العنوان (غير معروف أو جديد جدًا). اعتبره خطرًا.', report: 'تقرير الأمان', honeypot: 'HONEYPOT — قد لا تستطيع البيع', sellable: 'قابل للبيع', buyTax: 'ضريبة الشراء', sellTax: 'ضريبة البيع', source: 'الكود', verified: 'مُتحقَّق', notVerified: 'غير مُتحقَّق', noFlags: 'لا صلاحيات خطرة للمالك.', disclaimer: 'فحص آلي — ليس نصيحة مالية.',
    ...
    watchUsage: 'الاستخدام: /watch 0x… [تسمية] — عنوان EVM أو Solana أو Bitcoin.', watchInvalid: 'لا يبدو هذا عنوانًا صالحًا.', watchLimit: 'تتابع 10 عناوين بالفعل. احذف واحدًا بـ /unwatch.', watchAdded: 'تتم متابعة {a}. ستتلقى رسالة عند كل معاملة.', watchExists: 'تتابع هذا العنوان بالفعل.', watchNone: 'لا تتابع أي عنوان بعد. /watch 0x…', watchListTitle: 'العناوين المتابَعة', unwatchUsage: 'الاستخدام: /unwatch 0x…', unwatched: 'تمت الإزالة.',
    ...
    received: 'مستلَم', sent: 'مُرسَل', from: 'من', to: 'إلى', on: 'على',
  ...
  hi: {
    ...
    cmdApp: 'Telegram में वॉलेट खोलें', cmdPrice: 'लाइव मूल्य', cmdGas: 'अभी नेटवर्क शुल्क', cmdScan: 'honeypot / धोखाधड़ी जाँच', cmdWatch: 'हर लेन-देन पर संदेश', cmdAlert: 'मूल्य अलर्ट', cmdHelp: 'यह कैसे काम करता है', cmdLang: 'भाषा बदलें',
    ...
    watchUsage: 'उपयोग: /watch 0x… [लेबल] — EVM, Solana या Bitcoin पता।', watchInvalid: 'यह मान्य पता नहीं लगता।', watchLimit: 'आप पहले से 10 पते फ़ॉलो कर रहे हैं। /unwatch से एक हटाएँ।', watchAdded: '{a} फ़ॉलो हो रहा है। हर लेन-देन पर संदेश मिलेगा।', watchExists: 'आप यह पता पहले से फ़ॉलो कर रहे हैं।', watchNone: 'अभी कोई पता फ़ॉलो नहीं। /watch 0x…', watchListTitle: 'फ़ॉलो किए पते', unwatchUsage: 'उपयोग: /unwatch 0x…', unwatched: 'हटाया गया।',
    ...
    received: 'प्राप्त', sent: 'भेजा गया', from: 'से', to: 'को', on: 'पर',
  ...
  zh: {
    ...
    cmdApp: '在 Telegram 内打开钱包', cmdPrice: '实时价格', cmdGas: '当前网络费用', cmdScan: '蜜罐 / 诈骗检测', cmdWatch: '每笔交易发送消息', cmdAlert: '价格提醒', cmdHelp: '工作原理', cmdLang: '更改语言',
    ...
    scanUsage: '用法：/scan 0x…（EVM 代币或合约）。Solana：/scan &lt;mint&gt;', scanRunning: '🔍 正在通过 GoPlus Security 检查…', scanNoData: '该地址没有安全数据（未知或太新），请视为高风险。', report: '安全报告', honeypot: '蜜罐 — 你可能无法卖出', sellable: '可卖出', buyTax: '买入税', sellTax: '卖出税', source: '代码', verified: '已验证', notVerified: '未验证', noFlags: '未发现危险的所有者权限。', disclaimer: '自动检测 — 不构成投资建议。',
    flags: flags(['所有者可增发代币', '所有者可拉黑钱包', '转账可被暂停', '可升级代理（代码可变）', '隐藏所有者', '可自毁', '交易冷却时间', '无法全部卖出', '税率可更改', '转账中有外部调用']),
    watchUsage: '用法：/watch 0x… [标签] — EVM、Solana 或 Bitcoin 地址。', watchInvalid: '这看起来不是有效地址。', watchLimit: '你已关注 10 个地址，用 /unwatch 移除一个。', watchAdded: '正在关注 {a}。每笔交易都会收到消息。', watchExists: '你已关注该地址。', watchNone: '你还没有关注任何地址。/watch 0x…', watchListTitle: '关注的地址', unwatchUsage: '用法：/unwatch 0x…', unwatched: '已移除。',
    ...
    received: '收到', sent: '发出', from: '来自', to: '发往', on: '·',
  ...
  ja: {
    ...
    cmdApp: 'Telegram 内でウォレットを開く', cmdPrice: 'リアルタイム価格', cmdGas: '現在のネットワーク手数料', cmdScan: 'ハニーポット / 詐欺チェック', cmdWatch: '取引ごとにメッセージ', cmdAlert: '価格アラート', cmdHelp: '仕組み', cmdLang: '言語を変更',
    ...
    scanUsage: '使い方：/scan 0x…（EVM トークンまたはコントラクト）。Solana：/scan &lt;mint&gt;', scanRunning: '🔍 GoPlus Security で確認中…', scanNoData: 'このアドレスの安全性データがありません（不明または新しすぎます）。リスクありとして扱ってください。', report: 'セキュリティレポート', honeypot: 'ハニーポット — 売却できない可能性', sellable: '売却可能', buyTax: '購入税', sellTax: '売却税', source: 'コード', verified: '検証済み', notVerified: '未検証', noFlags: '危険なオーナー権限は見つかりません。', disclaimer: '自動チェック — 投資助言ではありません。',
    ...
    watchUsage: '使い方：/watch 0x… [ラベル] — EVM、Solana、Bitcoin のアドレス。', watchInvalid: '有効なアドレスに見えません。', watchLimit: 'すでに 10 件フォロー中です。/unwatch で 1 件外してください。', watchAdded: '{a} をフォロー中。取引ごとにメッセージが届きます。', watchExists: 'このアドレスはすでにフォロー中です。', watchNone: 'まだアドレスをフォローしていません。/watch 0x…', watchListTitle: 'フォロー中のアドレス', unwatchUsage: '使い方：/unwatch 0x…', unwatched: '削除しました。',
    ...
    received: '受信', sent: '送信', from: '送信元', to: '送信先', on: '·',
  ...
  ko: {
    ...
    cmdApp: 'Telegram 안에서 지갑 열기', cmdPrice: '실시간 가격', cmdGas: '현재 네트워크 수수료', cmdScan: '허니팟 / 사기 검사', cmdWatch: '거래마다 메시지', cmdAlert: '가격 알림', cmdHelp: '작동 방식', cmdLang: '언어 변경',
    ...
    scanUsage: '사용법: /scan 0x… (EVM 토큰 또는 컨트랙트). Solana: /scan &lt;mint&gt;', scanRunning: '🔍 GoPlus Security로 확인 중…', scanNoData: '이 주소의 보안 데이터가 없습니다 (알 수 없거나 너무 새로움). 위험한 것으로 간주하세요.', report: '보안 보고서', honeypot: '허니팟 — 매도가 불가능할 수 있음', sellable: '매도 가능', buyTax: '매수 세율', sellTax: '매도 세율', source: '코드', verified: '검증됨', notVerified: '미검증', noFlags: '위험한 소유자 권한이 없습니다.', disclaimer: '자동 검사 — 투자 조언이 아닙니다.',
    ...
    watchUsage: '사용법: /watch 0x… [라벨] — EVM, Solana 또는 Bitcoin 주소.', watchInvalid: '유효한 주소로 보이지 않습니다.', watchLimit: '이미 10개 주소를 팔로우 중입니다. /unwatch로 하나 제거하세요.', watchAdded: '{a} 팔로우 중. 거래마다 메시지를 받습니다.', watchExists: '이미 이 주소를 팔로우 중입니다.', watchNone: '아직 팔로우한 주소가 없습니다. /watch 0x…', watchListTitle: '팔로우 중인 주소', unwatchUsage: '사용법: /unwatch 0x…', unwatched: '제거됨.',
    ...
    received: '받음', sent: '보냄', from: '보낸 주소', to: '받는 주소', on: '·',
