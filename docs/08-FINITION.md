...
| ~~Signature Bitcoin non testée~~ | **RÉSOLU.** Deux obstacles, tous deux levés. (1) `@scure/btc-signer` est publié en ESM pur : `jest.config.js` le transforme désormais en CommonJS via `@babel/plugin-transform-modules-commonjs`, sur un périmètre réduit à ce paquet et à `micro-packed` (le motif doit être ANCRÉ — sans `^`, la négation est satisfaite par le `/node_modules/` imbriqué de btc-signer et le fichier repasse en « ignoré »). (2) La construction des signatures vivait dans `lib/walletStore`, derrière le déverrouillage : elle est extraite dans `src/domain/chains/btcSign`. BIP-137 et BIP-322 ont 23 tests, vérifiés DEPUIS LA SPEC (récupération de clé publique, recalcul indépendant du condensat, encodage VarInt aux frontières) et non en réutilisant le code testé. La construction de transaction en a 7 de plus, qui décodent le hex réellement diffusé : séquence RBF, sorties P2PKH et P2SH, conservation du montant à l'accélération. |
...
| ~~Token-2022 invisible~~ | **RÉSOLU.** `getSplTokens` n'interrogeait qu'un programme, donc tout jeton Token-2022 (PYUSD en tête) était invisible ; et le programme étant figé dans `solSpl`/`solPda`, il était aussi inenvoyable — le programme entre dans les SEEDS de l'ATA. Les deux programmes sont désormais interrogés et fusionnés, le programme du mint est résolu on-chain avant tout envoi (`getMintProgram`), et les frais de transfert de l'extension `transferFeeConfig` sont calculés et AFFICHÉS : sans eux l'app annonce « tu envoies 100 » alors que 99,5 arrivent. |
| ~~Historique SPL absent~~ | **RÉSOLU.** `parseSolanaTx` ne lisait que les lamports : envoyer 100 USDC produisait une ligne à ~0 SOL dont le « destinataire » était le compte ayant le plus gagné en lamports. `pre/postTokenBalances` sont lus, les comptes d'un même mint additionnés, la contrepartie déduite du mouvement inverse, et deux jetons qui bougent sont étiquetés `SWAP`. |
| **Testnet Bitcoin** | **ÉCARTÉ SCIEMMENT.** `bitcoin` est retiré des familles de réseaux personnalisés : `checkBtcAddress` refuse les adresses testnet (délibérément — pour qu'on ne puisse pas envoyer des fonds réels vers une adresse de test), donc la famille aurait donné un réseau créable et sélectionnable n'acceptant aucune adresse. Les réseaux `bitcoin` déjà enregistrés continuent de se charger. Pour l'ouvrir : préfixe `tb1`, versions base58 0x6F/0xC4, conditionnés à `config.testnet`. |
| ~~Poussière Bitcoin~~ | **RÉSOLU.** Aucun contrôle sur la sortie du DESTINATAIRE : la transaction était construite, signée, puis refusée à la diffusion (« non standard ») avec un message de nœud illisible. Et `DUST_SATS` portait la seule valeur P2WPKH (294) alors qu'on envoie désormais vers de l'hérité (546) et du Taproot (330). Seuils par type, appliqués avant signature. Ajouté aussi : les pièces non rentables sont ignorées (une entrée de 1 000 sats à 50 sat/vB coûte plus qu'elle n'apporte) et le nombre d'entrées est borné, au-delà la transaction n'est plus relayable. |
| ~~Réserve Bitcoin mensongère~~ | **RÉSOLU.** `estimateGasReserve` renvoyait 1 500 satoshis EN DUR tout en se déclarant estimée (`live: true`). Faux deux fois : rien n'était estimé, et 1 500 ne couvrent pas une transaction au-delà de ~10 sat/vB — le montant disponible était surévalué et un envoi du solde entier échouait sur « solde insuffisant », pour un montant que l'app venait de proposer. Calculée maintenant sur `getFeeRates()`. |
| ~~`approve` USDT~~ | **RÉSOLU.** USDT sur Ethereum (et quelques jetons antérieurs à la finalisation d'ERC-20) font échouer `approve(spender, montant)` tant que l'autorisation courante n'est pas nulle. Quelqu'un ayant déjà autorisé 100 USDT et voulant en échanger 200 voyait son échange échouer sur un revert inexpliqué. Remise à zéro préalable, uniquement quand l'autorisation est non nulle ET insuffisante — c'est-à-dire exactement le cas qui échouait. |
| ~~Destinataire Solana hors-courbe~~ | **RÉSOLU.** Toute chaîne de 32 octets était acceptée sans un mot. Or une adresse valide peut être une PDA — compte de jeton, compte de programme — que personne ne peut signer : y envoyer du SOL, c'est le perdre, et l'adresse d'un compte de jeton se copie aussi facilement que celle d'un portefeuille. `isWalletAddress` distingue les deux, l'écran d'envoi avertit. |
| **`ChainAdapter` est modelé sur l'EVM** | Bitcoin et Solana contournent l'interface : `prepareTransfer`, `signTransaction` et `broadcast` lèvent `NOT_SUPPORTED`, et tout passe par `sendBitcoin*` / `sendSolana*`. Fonctionnel aujourd'hui, mais ajouter TON ou Cosmos forcera à redessiner l'interface — à trancher AVANT de commencer, pas au milieu. |
