...
| 9 | Home morphing + pull-to-refresh Aura (§9, §10.4) | **partiel.** Fait du §10.4 : haptique de sélection au seuil, ambiance Synchronisation au lâcher (déjà portée par l'Aura), chiffres qui roulent dans le bon sens à la fin, spinner système rendu discret. **NON fait** : l'étirement du halo sous le doigt et le home morphing, qui demandent tous deux de piloter le défilement au geste — donc un `GestureHandlerRootView` à la racine et une réécriture du `ScrollView` de l'accueil. `react-native-gesture-handler` est bien dans le build (dépendance déclarée, module natif présent), donc c'est faisable en OTA — mais c'est une réécriture de l'écran principal, à faire seule et à tester sur device, pas en fin de série. |
